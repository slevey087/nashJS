...
//A temporary History that can be merged back in later.
//Dual represents an additional log that can be written to before
//the merge.
...
History.prototype.getType = function(type){
	return new History(this.filter(function(entry){
...
History.prototype.recurse = function(type){
	...
	return new History(list.map(function(entry){
		for (var key in entry) {
...
function UserHistory(history){
	...
	for (var method in History.prototype){
		if (isFunction(history[method])){
			if (method != "constructor") userHistory[method] = (function(method){
	
				return function(){
					var result = history[method].apply(history,arguments);
					if (result instanceof History) return new UserHistory(result);
					else if (isObject(result)) return JSON.parse(JSON.stringify(result));
					else return result;
				};
			})(method);
		}
...
gameHistory.scores = new History();
