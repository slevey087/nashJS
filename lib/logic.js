...
class RandomVariable extends Expression {
	...
	// Pick a number and then hold that number into the future.
	freeze() {
		var rv = this;
		var value = rv.expression();
		var expressionCache = rv.expression

		rv.value = value;
		rv.expression = function (unfreeze = false) {
			if (!unfreeze) return value;
			else if (unfreeze) {
				rv.expression = expressionCache;
				return rv.expression();
			}
		}
		rv.unfreeze = function () {
			delete rv.unfreeze
			rv.expression = expressionCache
			return rv.expression();
		}
	...
	// resume generating random numbers


	static creator(...args) {
		return new RandomVariable(...args)
	}
}
...
class ComplexVariable extends Variable {
	...
	static creator(...args) {
		return new ComplexVariable(...args)
	}
}
