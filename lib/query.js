...
function Query(shortcut, queryString, description, format = "shortcut") {

	// Only include requested properties, to avoid visual clutter
	if (format == "code" || format == "all") this.queryString = queryString;
...
Query.prototype.evaluate = function(target, ...args) {
	return evaluateQuery(this.queryString, target, ...args)
};
...
function QueryResult(queryString, result) {
...
// prototype contains .queryString (unless the result was just a string)
QueryResult.prototype.pack = function() {
	// If result is string,
	if (typeof this.result === "string") {
		var packed = new String(this.result)
		packed.queryString = this.queryString
		return packed;
	}
	// If it's array
