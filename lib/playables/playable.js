...
class _Playable {
	...
	checkInit({ initializePlayers } = {}, result) {
		...
		if (initializePlayers) {
			...
			if (initializePlayers === true) reinitializePlayers("all")

			// if it's a playerList, use that
			else if (initializePlayers instanceof PlayerList) reinitializePlayers(initializePlayers)
			...
			// if we get a function, then run the function and check that it's returning a playerList
				...
				if (list instanceof PlayerList) reinitializePlayers(list);
			}

			// otherwise, convert it to a playerList and let PlayerList deal with it.
			else reinitializePlayers(new PlayerList(initializePlayers));
		}
...
let Playable = (function () {
	...
	class Playable extends CallableInstance {
		...
		play(parameters = {}) {
			...
			else if (gameHistory.stop) log.warn(".play was called but the history has ended. The history must be reset in order for play to continue.")
			...
			var returnValue = Promise.resolve()
				...
				.catch(function (reason) {
					...
					//If the game was stopped because everybody's dead, we'll end up here, and things are fine. Just log it.					
					if (reason.result == "Population Collapse")
						return Promise.resolve(reason.result);
					else {
						gameHistory.log.add({ error: reason });
						return Promise.reject(reason);
					}
				})
				.then(function (result) {
					// clear callingPlayable, so next run is clean
					perfectInformation.clearCallingPlayable();

					// Return information to use (do not return the "result" from above, as it has _playables in it)
					return Promise.resolve({
