...
var {outcomeTreeAddAll,
		outcomeTreeGetValue} = require('../helperFunctions')("playable");
...
_Playable.prototype.checkInit = function({initializePlayers=false}={}, result){
	return initializePlayers ? reinitializePlayers(result) : Promise.resolve(result);
};
...
_Playable.prototype.summarize = function(summary = {}, entries={}, shortCircuit=false, maxEntries=10){
	...
	if (!shortCircuit || shortCircuit !== this) this.summaryNext(summary, entries)
...
playablePrototype.constructor = function (_playable) {
	...
	this.play = function({initializePlayers=false,usePayoffs=true, shortCircuit=false, writeHistory=true, clearHistory=true, releasePlayers=true}={}){
		...
		var history = writeHistory ? _playable.history || gameHistory : new History();
