...
_Sequence.prototype.play = function({history=gameHistory}={}){
	...
	var action = function action(result){
		...
		if (result.playable !== sequence.playableFinish) {
			...
			if (result.playable.findNext({result}).length > 0) {
				...
				return result.playable.playNext(result, {shortCircuit:true, history:subHistory})
	...
	return sequence.playableStart.play({shortCircuit:true, history:subHistory}) 
	.then(action)
...
_Sequence.prototype.handleHistory = function({history=gameHistory}={},result){
...
_Sequence.prototype.summaryThis = function(summary, entries, shortCircuit){
	...
	this.playableStart.summarize(summary.action, entries, shortCircuit=this.playableFinish)
}
