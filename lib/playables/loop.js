...
//Backend class for Loop
class _Loop extends _Playable {
	constructor(id, playable, count, parameters = {}) {
		super(id, parameters)

		this.playable = playable;
		this.logContinue = parameters.logContinue || false
		this.playableParameters = parameters.parameters || {};

		this.count = count;
	}

	play({ history, information, playableParameters = {} } = {}) {
		// merge received with stored parameters
		playableParameters = { ...this.playableParameters, ...playableParameters }
...
// Frontend class for Loop
class Loop extends Playable {
	constructor(playable, count = 1, parameters = {}) {
		var id = idHandler(parameters.id, "loop");

		// find playable or throw
		if (playable instanceof Playable) playable = registry.playables[playable.id()]
		else throw new Error("Argument 'playable' must be a playable")

		// count must be number
		if (isNaN(count)) throw new Error("Argument 'count' must be a number.")
...
function Loop(playable, count = 1, parameters = {}) {
	...
	//Create backend loop object
	var _loop = new _Loop(id, registry.playables[playable.id()], count, parameters);
