	...
	class TurnBranch extends Branch {
		...
		payoff(payoffs) {
			...
			if (implicit.length == numPlayers && implicit.every(function (payoff) {
				...
				_turn.payoffsImplicit.setValue(path, implicit);
			...
			payoffs.slice(numPlayers).forEach(function (explicit) {
				if (isObject(explicit))
					_turn.payoffsExplicit.setValue(path, explicit);
			});
...
class _Turn extends _Playable {
	constructor(id, decisions, parameters = {}) {
		...
		// If no Range's, use OutcomeTrees for discrete choices.
		else {
			turn.branchMode = "tree"

			log("silly", "Adding implicit payoffs map to turn.");
			turn.payoffsImplicit = new OutcomeTree(turn.decisionMap, null, function () {
				return Array(turn.decisionMap.length).fill(0);
			});

			log("silly", "Adding explicit payoffs map to turn.");
			turn.payoffsExplicit = new OutcomeTree(turn.decisionMap, null, function () {
				return {};
			});

			log("silly", "Adding blank next map to turn.");
			turn.next = new OutcomeTree(turn.decisionMap, null, function () {
				return [];
			});
		}
	...
	summaryThis(summary) {
		...
		if (this.branchMode == "tree")
			summary("payoffs", JSON.parse(
				JSON.stringify({
					implicit: this.payoffsImplicit.collapse(),
					explicit: this.payoffsExplicit.collapse()
				})
			))
	...
	play({ usePayoffs, history, information, releasePlayers = true } = {}) {
		var turn = this;
		...
		var compileInformation = function () {
		...
		return Promise.all(
			...
			.then(function (result) {
				//Implement payoffs
				if (usePayoffs) {
					var implicitPayoffs = turn.payoffsImplicit.getValue(result.result);
					var explicitPayoffs = turn.payoffsExplicit.getValue(result.result);
	...
	findNext(result = {}) {
		...
		if (this.branchMode == "tree") {
			next = this.next.getValue(result.result);
		}
	...
	generateBranches() {
		...
		var branchTree = new OutcomeTree(_turn.decisionMap, null, function (path) {
			let branch = new TurnBranch(path, _turn)
			_turn.branches.push(branch)
			return branch
		})
		branchTree.collapse(turn)
				...
				if (input.length == 1) {
					...
					*/
					...
					if (
						...
						turn.payoffsImplicit.setValue(splitPath, implicit);
					}

					//Any remaining should be assigned as explicit payoffs, if they're objects.
					splitPayoffs.slice(numPlayers).forEach(function (explicit) {
						if (isObject(explicit))
							turn.payoffsExplicit.setValue(splitPath, explicit);
					});
				}
...
class Turn extends Playable {
	...
	payoffs() {
		...
		return JSON.parse(
			JSON.stringify({
				implicit: _turn.payoffsImplicit.collapse(),
				explicit: _turn.payoffsExplicit.collapse()
			})
		);
	};
	...
	payoffsMatrix() {
		...
		var mapper = function (obj, path = []) {
			...
			if (Array.isArray(obj)) {
				...
				var explicit = _turn.payoffsExplicit.getValue(path);

				// Only add an entry if the explicit payoffs object is not empty
				return Object.keys(explicit).length > 0 ? payoff.concat([_turn.payoffsExplicit.getValue(path)]) : payoff
			}

			// Otherwise, dig in deeper
			else return Object.keys(obj).map(function (key) { return mapper(obj[key], path.slice(0).concat([key])) })
		}
		return mapper(_turn.payoffsImplicit.collapse())
	}
}
