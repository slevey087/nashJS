...
function _Turn(id, choices, parameters={}){
	...
	recurse(turn.choiceMap, turn.payoffsImplicit,null, function(){return [];})
	.then(function(result){
		...
		return recurse(turn.choiceMap, turn.next,null, function(){return [];});
	}).then(function(result){
...
_Turn.prototype.play = function({usePayoffs=true, history=gameHistory, information=PerfectInformation, releasePlayers=true}={}){
	...
	var compileInformation = function(ri){
		console.log(JSON.parse(JSON.stringify(choiceInformation)));
		console.log("equal? ", (ri == choiceInformation));
		...
		choiceInformation.additional[0].turn.choices.push(choiceInformation.additional.pop())
		
		console.log("choice information: ", JSON.parse(JSON.stringify(choiceInformation)));
		information.additional.forEach(choiceInformation.addAdditional.bind(choiceInformation));
	};
	
	return Promise.all(turn.choices.map(function(choice){
		
		return choice.play({shortCircuit:true, history:choiceHistory, information:choiceInformation, _compileInformation:compileInformation, releasePlayers:false});
...
_Turn.prototype.setAllPayoffs = function(payoffArray){
	...
	function recurse(input, numPlayers, payoffs, path=[], coordinates=[]){
		return SynchronousPromise.resolve(path).then(function(path){
			...
			return SynchronousPromise.all(input[0].map(function(item, index){
				...
				if (input.length == 1) {
					...
					*/
					...
					if (implicit.length == numPlayers && implicit.every(function(payoff){return !isNaN(payoff);})) {
					...
					splitPayoffs.slice(numPlayers).forEach(function(explicit){
						if (isObject(explicit)) outcomeTreeSetValue(turn.payoffsExplicit, splitPath, explicit); 
					}); 
				}
				...
				return recurse(input.slice(1), numPlayers,splitPayoffs, splitPath, splitCoordinates);
			}));		
		});
	}
	
	return recurse (turn.choiceMap, turn.choices.length, payoffArray).catch(function(reason){log("error", reason);});
};
...
_Turn.prototype.summaryThis = function(summary, entries, shortCircuit=false){
	...
	this.choices.forEach(function(choice, index){
		summary.choices[index] = choice.summarize(summary.choices[index], entries, true);
	});
	
	// Include payoffs
	summary.payoffs = JSON.parse(JSON.stringify({
