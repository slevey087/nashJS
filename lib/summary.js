...
class Summary extends CallableInstance {
	...
	tree(branchName, treeMap, outcomeTree, func, treeIfIdentical = false) {
		var summary = this;
		var items = []

		var tempTree = new OutcomeTree(treeMap, null, function (path) {
			var item = outcomeTree.getValue(path)
			items.push(item)
		...
		else summary.summary[branchName] = tempTree.collapse();
	...
	treeArray(branchName, treeMap, outcomeTree, func, { arrayIfOne = false, treeIfIdentical = false } = {}) {
		var summary = this;
		var items = []
		...
		var tempTree = new OutcomeTree(treeMap, null, function (path) {
			var array = outcomeTree.getValue(path)
			items.push(array)
