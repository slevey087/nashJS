...
_Choice.prototype.play = function({initializePlayers=false, usePayoffs=false, shortCircuit=false}={}){
	...
	return Promise.resolve().then(function(){
		...
		if (choice.next[result][0] instanceof _Playable) return Promise.all(choice.next[result].map(function(item){return item.play();}));
...
_Turn.prototype.play = function({usePayoffs=true, initializePlayers=false, shortCircuit=false}={}){
	...
	return Promise.all(turn.choices.map(function(choice){
		...
		if (outcomeValue[0] instanceof _Playable) return Promise.all(outcomeValue.map(function(playable){return playable.play();}));
...
_Loop.prototype.play = function({initializePlayers=false, shortCircuit=false}={}){
	...
	return promise.then(function(result){
		...
		//If there's somewhere to go, then go.
		if (loop.next[0] instanceof _Playable) return Promise.all(loop.next.map(function(playable){return playable.play();}));
...
_SLoop.prototype.play = function({initializePlayers=false, shortCircuit=false}={}){
	...
	return promise.then(function(result){
		...
		//If there's somewhere to go, then go.
		if (sLoop.next[0] instanceof _Playable) return Promise.all(sLoop.next.map(function(playable){return playable.play();}));
